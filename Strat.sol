...
contract StrategyMasterChefPolygonLP is StratManager, FeeManager {
    ...
    // Routes
    address[] public outputToUSDCRoute = [output, usdc];
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (tx.origin == owner() || paused()) {
            ...
            IERC20(want).safeTransfer(strategist, wantBal.sub(withdrawalFee));
        }
    }
    ...
    function chargeFees() internal {
        uint256 Fees = IERC20(output).balanceOf(address(this)).mul(STRATEGIST_FEE).div(MAX_FEE); // this will create buying pressure for Meso
        ...
        IERC20(usdc).safeTransfer(strategist, Fees);
    }
    ...
    function getHarvestable() external view returns (uint256){
        return IMasterChef(output).pendingCake(poolId, address(this));
    }
